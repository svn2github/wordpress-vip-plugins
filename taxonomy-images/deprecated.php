...
function taxonomy_images_plugin_shortcode_deprecated( $atts = array() ) {
	...
	if ( ! taxonomy_exists( $taxonomy ) ) {
		$tax = wp_strip_all_tags( trim( $taxonomy ) );
	...
	if ( ! is_wp_error( $terms ) ) {
		foreach( (array) $terms as $term ) {
			...
			if ( 'grid' == $template ) {
				$o .= "\n\t" . '<div class="taxonomy_image_plugin-' . esc_attr( $template ) . '">';
				$o .= "\n\t\t" . '<a style="float: left;" title="' . esc_attr( $title_attr ) . '" href="' . esc_url( $url ) . '">' . wp_kses_post( $img ) . '</a>';
				$o .= "\n\t" . '</div>';
			} else {
				$o .= "\n\t\t" . '<a title="' . esc_attr( $title_attr ) . '" href="' . esc_url( $url ) . '">' . wp_kses_post( $img ) . '</a>';;
				$o .= "\n\t\t" . '<h2 style="clear: none; margin-top: 0; padding-top: 0; line-height: 1em;"><a href="' . esc_url( $url ) . '">' . esc_html( $title ) . '</a></h2>';
				$o .= $description;
