...
function ap_wibiya_mobile_options_page()
{
	...
	if((($GLOBALS['ap_wibiya_mobile_policy']>=__POLICY_DISABLE_USER__) && ($GLOBALS['ap_wibiya_mobile_register']!=1))||((get_option('ap_wibiya_mobile_url')=="")&&$GLOBALS['ap_wibiya_mobile_register']==1))
...
function ap_wibiya_check_recovery()
{
	...
	if($responseRec['register']==1&&(!empty($responseRec['url'])))
	{
		$GLOBALS['ap_wibiya_mobile_register'] = 1;
		update_option('ap_wibiya_mobile_register', 1);
		$url=base64_decode($responseRec['url']);
		if($url!="")
		{ update_option('ap_wibiya_mobile_url',$url ); }
	}
}
