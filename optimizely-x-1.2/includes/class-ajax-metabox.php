<?php
/**
 * Optimizely X: AJAX_Metabox class
 *
 ...
 * the API via the Optimizely X Metabox
 *
 * @since 1.0.0
 */
class AJAX_Metabox extends AJAX {
	...
	private function build_targeting_page( $project_id, $post ) {
		...
		if ( 'conditional' === get_option( 'optimizely_x_activation_mode' ) ) {
			$targeting_page['activation_type'] = 'polling';

			// Try to set conditional activation code.
			$activation_code = get_option( 'optimizely_x_conditional_activation_code' );
