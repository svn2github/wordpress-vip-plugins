( function( $ ) {
  ...
  function ndnSelectVideo(e) {
    ...
    values['ndn-config-widget-id'] = values['ndn-video-start-behavior'];


     values['data-config-pb'] = values['ndn-video-start-behavior'] ;
    if(values['ndn-video-start-behavior'] =='2')
    {
      values['ndn-config-widget-id']='2';
    }
   else{
    
        values['ndn-config-widget-id']='1'
    }
 ...
 var buildHtml = jQuery( '<img />' )
    ...
    var html_image = buildHtml.prop('outerHTML');

    /*Build embeded div*/

    if((values['ndn-video-start-behavior']=='1') || (values['ndn-video-start-behavior']=='2')){
    var buildHtml_div = jQuery( '<div></div>' )
      .addClass( 'ndn_video')
      .css({
        'max-width': values['ndn-video-width'] + 'px',
        'max-height': values['ndn-video-height'] + 'px',
      ...
      .attr({
        ...
        'data-config-playlist-id':values['ndn-default-play-list-id']
      });
    }
    else{
var buildHtml_div = jQuery( '<div></div>' )
      ...
      .attr({
        ...
        'ndn-video-height': values['ndn-video-height'],
        'data-config-playlist-id':values['ndn-default-play-list-id'],
        'data-config-pb': values['data-config-pb']
      });
     ...
     var html_div = buildHtml_div.prop('outerHTML');
    

   var html_script='[embedScript]';

   var html=html_image+ " "+ html_div+ " "+ html_script; 

   var contentHtml = $(".ndn-image-embed").html();
$(".ndn-image-embed").after(contentHtml);
$(".ndn-image-embed").remove();
