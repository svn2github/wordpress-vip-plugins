...
(function($) {
	...
	media.view.GettyDownloadAuthorizations = media.View.extend({
		...
		updateSelectedDownloadSize: function(ev) {
			var selectedSizeName = $(ev.target).val();
			...
			var selectedSize = _.find(sizesForSelectedAgreement, function(size) { return size.name === selectedSizeName; });
	...
	media.view.GettyToolbar = media.view.Toolbar.extend({
		...
		insertImage: function() {
			...
			getty.tracking.asset.id = image.get('ImageId');
			getty.tracking.asset.family = image.get('ImageFamily');
		...
		updateButton: function() {
			if(this.collection.length == 0) {
				...
				if(this.controller.state().get('mode') == 'embed') {
					...
					this.button.$el.addClass('embed-image-button');
				}
				else {
					if(image.get('attachment')) {
						this.button.model.set('text', getty.text.insertImage);
						this.button.$el.addClass('insert-image-button');
					}
					else {
						this.button.model.set('text', getty.text.insertComp);
						this.button.$el.addClass('insert-comp-button');
					}
				}
			}
		},
	...
	media.view.GettyModeSelect = media.View.extend({
		...
		initialize: function() {
			...
			media.editor.add('content').on('close',function(){ getty.user.settings.unset('mode'); });
