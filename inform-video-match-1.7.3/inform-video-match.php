...
function ndnreplace_image_from_dev($content){

$dom = new DOMDocument(); 
$dom->loadHTML($content);
$xpath = new DOMXpath($dom);
$data_x_path=$xpath->query('//img[@class="ndn_embed"]');
if(count($data_x_path)>0){
foreach ($data_x_path as $img) {

  $new_img = replace_ndn_image($img, $width = '566', $height = '576');
...
$new_html = $dom->saveHTML();
 $new_html2 = preg_replace('~<(?:!DOCTYPE|/?(?:html|head|body))[^>]*>\s*~i', '', $new_html);
 return $new_html2;
}else{
return $content;
}
...
add_filter( 'the_content', 'ndnreplace_image_from_dev' );


function replace_ndn_image($img, $width, $height) {

  $data_config_video_id = $img->getAttribute('data-config-video-id');
  $data_config_widget_id = $img->getAttribute('data-config-widget-id');
  $data_config_tracking_group = $img->getAttribute('data-config-tracking-group');
  $data_config_site_section = $img->getAttribute('data-config-site-section');
  $ndn_responsive = $img->getAttribute('ndn-responsive');
  $data_config_width = $img->getAttribute('data-config-width');
  $data_config_height = $img->getAttribute('data-config-height');
  $data_config_playlist_id = $img->getAttribute('data-config-playlist-id');
  $data_config_pb = $img->getAttribute('data-config-pb');
  $data_config_aspect_ratio = $img->getAttribute('data-config-aspect-ratio');
  $data_config_type = $img->getAttribute('data-config-type');


 
  $new_img = '<div class="ndn_embed" data-config-video-id="'.$data_config_video_id.'" 
  data-config-widget-id="'.$data_config_widget_id.'"
  data-config-tracking-group="'.$data_config_tracking_group.'"
  data-config-site-section="'.$data_config_site_section.'"
  ndn-responsive="'.$ndn_responsive.'"
  data-config-width="'.$data_config_width.'"
  data-config-height="'.$data_config_height.'"
  data-config-playlist-id="'.$data_config_playlist_id.'"
  data-config-pb="'.$data_config_pb.'" data-config-aspect-ratio="'.$data_config_aspect_ratio.'" data-config-type="'.$data_config_type.'"></div><script type="text/javascript">var _informq = _informq || []; _informq.push(["embed"]);</script>';
...
class NDN_Plugin
{
  ...
  private function define_public_hooks()
  {
      ...
      $this->loader->add_action('wp_enqueue_scripts', $plugin_public, 'ndn_plugin_hook_embed');
      $this->loader->add_filter('script_loader_tag', $plugin_public, 'add_scriptid_attribute', 10, 2);
  }
