...
class NDN_Plugin_Admin
{
    ...
    public static $settings_form_options = array(
        ...
        'ndn_default_playlist_id' => 'ndn-default-playlist-id',
    );
    ...
    public static $start_behavior_options = array(
        ...
        'scrool_to_play' => array(
            'name' => 'Scroll to Play',
            'value' => '7',
        ),
        'defer_tracking_roup_level' => array(
            'name' => 'Preset By DPID',
            'value' => '10',
        ),
    ...
    public function post_page_enqueue_stylesheet($hook) {
        ...
        if ( $hook == 'admin_page_inform-video-search-results?' ) {

            wp_enqueue_style('ndn_plugin_admin_search_results_css', NDN_PLUGIN_DIR . '/css/ndn_plugin_admin_search_results.css', $this->version, 'all' );
        }

       if ( $hook == 'admin_page_inform-video-search?' ) {

            wp_enqueue_style( 'ndn_plugin_admin_custom_modal', NDN_PLUGIN_DIR . '/css/ndn_plugin_admin_custom_modal.css', $this->version, 'all' );
       }
        
       if ( $hook == 'admin_page_inform-plugin-login?' ) { 
        
         wp_enqueue_style('ndn_plugin_admin_login', NDN_PLUGIN_DIR . '/css/ndn_plugin_admin_login.css', $this->version, 'all' );
    ...
    public function enqueue_scripts()
    {
        ...
        if ( $post ) {
          wp_localize_script( $this->plugin_name, 'NDNAjax', array( 'postID' => $post->ID) );
        }
    }
    /**
    ...
    public function post_page_enqueue_scripts($hook)
    {
        ...
        if ( $hook == 'admin_page_inform-video-search-results?' ) {
            wp_enqueue_script('launch_newsinc', 'https://launch.newsinc.com/js/embed.js', array(), '', false);
            wp_enqueue_script( 'ndn_plugin_admin_search_results', NDN_PLUGIN_DIR . '/js/ndn_plugin_admin_search_results.js', array( 'jquery' ), $this->version, false );
        }

        if ( $hook == 'admin_page_inform-video-search?' || $hook == 'admin_page_inform-plugin-login?' ) { 
          wp_enqueue_script( 'ndn_plugin_admin_custom_modal', NDN_PLUGIN_DIR . '/js/ndn_plugin_admin_custom_modal.js', array( 'jquery' ), $this->version, false );
        }   
    ...
    /* add id into the script tag */

    public function add_scriptid_attribute($tag, $handle) {

      // add script handles to the array below
       $scripts_to_async = array('launch_newsinc');
       
       foreach($scripts_to_async as $async_script) {
          if ($async_script === $handle) {
             return str_replace(' src', ' id="_nw2e-js" async="async" src', $tag);
          }
       }
       return $tag;
    }
    ...
    /**
     * Create Plugin Setting page and Menu item oAdmin Page.
     */
    ...
    public function submit_client_information()
    {
        ...
        if (!isset( $_POST['ndn_setting_nonce_field'] ) || ! wp_verify_nonce( $_POST['ndn_setting_nonce_field'], 'ndn_setting_nonce_action' )) {
           return false;
     ...
     * @param array $response API response from getting token call
     */
    ...
    public function notify_user_for_configuration()
    {
        ...
        <div class="update-nag">
            <?php
            if ( current_user_can( 'manage_options' ) ) {
                echo wp_kses_post(sprintf(__( 'Your tracking group needs to be entered in <a href="%s" title="Inform Video Match Settings" class="ndn-notify-settings">Inform Video Match Settings</a>.', 'Inform' ), esc_url( $url) ));
            } else {
                echo wp_kses_post(sprintf(__( 'Please contact your administrator to activate the Inform Video Match plugin.', 'Inform' ), esc_url( $url) ));
            }
            ?>
    ...
    public static function save_plugin_settings()
    {
        ...
        if (!isset( $_POST['ndn_save_nonce_field'] ) || ! wp_verify_nonce( $_POST['ndn_save_nonce_field'], 'ndn_save_nonce_action' )) {
           return false;
    ...
    public function submit_search_query()
    {
        ...
        if (isset( $_POST['search-action']) && '1' == $_POST['search-action']) {
            ...
            $sortByVideo = sanitize_text_field( $_POST['sort-by-video-type'] );   
            $video_type = sanitize_text_field( $_POST['search-video-type'] ); 
            $sort_by_upload_date = sanitize_text_field( $_POST['sort-by-upload-date'] ); 
            $sort_by_min_date = sanitize_text_field( $_POST['sort-by-min-date'] ); 
            $sort_by_max_date = sanitize_text_field( $_POST['sort-by-max-date'] ); 
            $sort_by_video_duration = sanitize_text_field( $_POST['sort-by-video-duration'] ); 

            self::save_option( 'ndn_search_query', $query );
            self::save_option( 'ndn_search_video_type', $search_video_type );
            self::save_option( 'ndn_sort_video_type', $sortByVideo );
            self::save_option( 'ndn_sort_video_upload_date', $sort_by_upload_date );

            self::save_option( 'ndn_sort_min_date', $sort_by_min_date );
            self::save_option( 'ndn_sort_max_date', $sort_by_max_date );
            self::save_option( 'ndn_sort_video_duration', $sort_by_video_duration );

            if($sort_by_upload_date == 'all'){
                $upload_date = '';
            }
            elseif($sort_by_upload_date == 'Last 24 hrs'){
                $upload_date = date('Y-m-d\TH:i:s\Z', strtotime('-24 hour'));
            }
            elseif($sort_by_upload_date == '2 days'){
                $upload_date = date('Y-m-d\TH:i:s\Z', strtotime('-2 day'));
            }
            elseif($sort_by_upload_date == 'Last week'){
                $upload_date = date('Y-m-d\TH:i:s\Z', strtotime('-1 week'));
            }
            elseif($sort_by_upload_date == 'Last month'){
                $upload_date = date('Y-m-d\TH:i:s\Z', strtotime('-1 month'));
            }
            $response = $this->run_text_search( $access_token, $query, $video_type, $sortByVideo, $upload_date, $sort_by_min_date, $sort_by_max_date, $sort_by_video_duration );
            if ( !is_wp_error( $response ) && is_array( $response ) ) {
   ...
   private function run_text_search( $access_token, $search_string, $video_type, $sortByVideo, $sort_by_upload_date, $sort_by_min_date, $sort_by_max_date ,$sort_by_video_duration )
    {
         // if ( WP_CACHE ) {
        //     // Cache Key
            $cache_key = 'ndn_search_results_' . $search_string;
        ...
        if($sort_by_min_date){
           $sort_by_upload_date = date('Y-m-d\TH:i:s\Z', strtotime($sort_by_min_date)); 
        }
              
        if($sort_by_max_date){
            $max_date = date('Y-m-d\TH:i:s\Z', strtotime($sort_by_max_date));
        }
        else{
             $max_date = '';
        }
        if($sortByVideo == 'relevancy'){
            $sortingVideo = 'relevancy';
        }
        else{
            $sortingVideo = 'recency';
        }
        $query_data = array(
            'text' => $search_string,
            'fieldList' => $video_type,
            'sort' => $sortingVideo,
            'pubDateMin' => $sort_by_upload_date,
            'pubDateMax' => $max_date,
            'minDuration' => $sort_by_video_duration,
            'rows' => '20',
        );
    ...
    public function set_featured_image()
    {
        ...
        if ( is_wp_error( $image_id ) ) {
            wp_die( 'This image can not set as featured image.' );
        } else {
            // Set featured image
