...
<form name="ndn-search" class="ndn-search-form" action="<?php echo esc_attr( 'admin.php' ) ?>" method="post" accept-charset="UTF-8" analytics-category="WPSearch" analytics-label="SearchAgain" novalidate>
   <select id="ndn-textlist" class="ndn-select-button" style="vertical-align: inherit;" name="search-video-type">
    <option value="" <?php if(get_option( 'ndn_search_video_type' )=='text') echo 'selected="selected"'; ?>>All Text</option>
    <option value="description" <?php if(get_option( 'ndn_search_video_type' )=='description') echo 'selected="selected"'; ?>>Description</option>
    <option value="title" <?php if(get_option( 'ndn_search_video_type' )=='title') echo 'selected="selected"'; ?>>Title</option>
    <option value="id" <?php if(get_option( 'ndn_search_video_type' )=='id') echo 'selected="selected"'; ?>>Video ID</option>
  </select>
  ...
  <select id="videotypelist" name="videotypelist" class="ndn-select-button">
    <option value="recency" <?php if(get_option( 'ndn_sort_video_type' )=='recency') echo 'selected="selected"'; ?>>Latest Videos</option>
    <option value="relevancy" <?php if(get_option( 'ndn_sort_video_type' )=='relevancy') echo 'selected="selected"'; ?>>Relevance</option>
  </select>
  </td>
  <td width="26%">
  <select name="ndn-upload-date" class="ndn-select-button" id="ndnVideoUploadDate">
    <option value="all" <?php if(get_option( 'ndn_sort_video_upload_date' )=='all') echo 'selected="selected"'; ?>>All</option>
    <option value="Last 24 hrs" <?php if(get_option( 'ndn_sort_video_upload_date' )=='Last 24 hrs') echo 'selected'; ?>>Last 24 hrs</option>
    <option value="2 days" <?php if(get_option( 'ndn_sort_video_upload_date' )=='2 days') echo 'selected="selected"'; ?>>2 days</option>
    <option value="Last week" <?php if(get_option( 'ndn_sort_video_upload_date' )=='Last week') echo 'selected="selected"'; ?>>Last week</option>
    <option value="Last month" <?php if(get_option( 'ndn_sort_video_upload_date' )=='Last month') echo 'selected="selected"'; ?>>Last month</option>
  </select>
  </td>
  <td>
  ...
  <div id="ndn-advance-search">
    ...
    <select id="ndn-video-duration-list">
      <option value="" <?php if(get_option( 'ndn_sort_video_duration' )=='') echo 'selected="selected"'; ?>>All</option>
      <option value="30" <?php if(get_option( 'ndn_sort_video_duration' )=='30') echo 'selected="selected"'; ?>>30 Secs</option>
      <option value="60" <?php if(get_option( 'ndn_sort_video_duration' )=='60') echo 'selected="selected"'; ?>>1 Min</option>
      <option value="120" <?php if(get_option( 'ndn_sort_video_duration' )=='120') echo 'selected="selected"'; ?>>2 Mins</option>
     <option value="600" <?php if(get_option( 'ndn_sort_video_duration' )=='600') echo 'selected="selected"'; ?>>10 Mins</option>
     <option value="601" <?php if(get_option( 'ndn_sort_video_duration' )=='601') echo 'selected="selected"'; ?>>over 10 Mins</option>
    </select></br>
            ...
            <section>
              <fieldset>
                <label>
                  ...
                  <div>
                     <input class="ndn-playlist-id" name="<?php echo esc_attr( self::$settings_form_options['ndn_default_playlist_id'] ) ?>" type="text" value="<?php echo esc_attr( get_option( 'ndn_default_playlist_id' ) ? get_option( 'ndn_default_playlist_id' ) : '') ?>" />
                  </div>
                </label>
              </fieldset>
            </section>
