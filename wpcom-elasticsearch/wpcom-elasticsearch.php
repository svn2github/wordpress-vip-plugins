...
class WPCOM_elasticsearch {
	...
	public function filter__posts_request( $sql, $query ) {
		...
		// Enforce Post Status on Searches
		// This is required as VIP2_ Indexes have draft and non-published content indexed
		// Defaults to "publish" only but can be filtered by `wpcom_elasticsearch_post_status` filter
		$post_status = [ 'publish' ];
