		...
		if( !empty( $atts['player_id'] ) ) {
			...
			if( $player_version === 'v4' ) {
				...
				if( !$v4_scripts ) {
					...
					$scripts = array_merge( array( $url_base . 'core.min.js' ), $analytics_scripts );
					...
					if( $settings['ad_plugin'] ) {
						...
						if( isset( $this->special_ads[$settings['ad_plugin']] ) ) {
							...
							$ad_plugin = $url_base . 'ad-plugin/' . $settings['ad_plugin'];
						}
					...
					$scripts[] = $url_base . 'skin-plugin/html5-skin.min.js';
					...
					<link rel="stylesheet" href="<?php echo esc_url( $url_base . 'skin-plugin/html5-skin.min.css' ); ?>" />
