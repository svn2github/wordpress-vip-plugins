...
class Livefyre_Domain {
    ...
    public function set_pull_url( $url_template ) {
        $request_url = 'https://' . $this->get_host() . '/?pull_profile_url=' . urlencode($url_template) . '&actor_token=' . $this->user('system')->token();
    ...
    public function set_user_affiliation( $user_id, $type, $scope = 'domain', $target_id = null ) {
        ...
        if ( !in_array( $type, $allowed_types ) ) {
            ...
            $systemuser = $this->user( 'system' );
            $request_url = 'https://' . $this->get_host() . '/api/v1.1/private/management/user/' . $user_jid . '/role/?lftoken=' . $this->user('system')->token();
            $post_data = array(
                'affiliation' => $type
            );
            if ($scope == 'domain') {
    ...
    public function source_js_v1() {
        return '<script type="text/javascript" src="https://zor.' . $this->get_livefyre_tld() . '/wjs/v1.0/javascripts/livefyre_init.js"></script>';
    }

    public function source_js_v3() {
        return '<script type="text/javascript" src="https://zor.' . $this->get_livefyre_tld() . '/wjs/v3.0/javascripts/livefyre.js"></script>';
    }
