...
Description: This plugin adds a most popular feed. It attaches to the rss2_head hook and alters the query according to the needs. It uses the stats_part() functionality provided in wp-content/mu-plugins/stats.php to determine top posts. Alterations of the feedoutput can be done using one of the various hooks provided in wp-includes/feed-rss2.php (see example below). This plugin is coded for WordPress.com VIP setups. $_GET - parameters "includepages", "limit" (limit <=100 ) exist to alter content of rss feed. $_GET - paramter "forceupdate=1" updates the rewrite_rules option and "printrules=1" provides output of the existing rewrite rules for siteadmins so they can be copied easily.
Author: Thorsten Ott
*/
...
function mostpopular_feed_query( $args = array() ) {
    ...
    query_posts( array( "post__in" => $filtered_post_ids, "orderby" => "none" ) );
