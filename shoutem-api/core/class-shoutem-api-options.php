...
class ShoutemApiOptions {
	...
	private function update_options(&$options) {
		...
		if(!empty($_REQUEST['encryption_key'])) {
			$options['encryption_key'] = sanitize_text_field($_REQUEST['encryption_key']);
		}

		if(array_key_exists('cache_expiration',$_REQUEST)) {
			$expiration = sanitize_text_field($_REQUEST['cache_expiration']);
		...
		if(array_key_exists('lead_img_custom_field_regex',$_REQUEST)) {
			$options['lead_img_custom_field_regex'] = sanitize_text_field($_REQUEST['lead_img_custom_field_regex']);
		}
