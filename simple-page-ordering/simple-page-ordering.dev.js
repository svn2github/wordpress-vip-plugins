function update_simple_ordering_callback(response) {
	if ( response == 'children' ) { window.location.reload(); return; }

	var changes = jQuery.parseJSON(response);

	if ( changes['id'] != null ) {
		jQuery.post( ajaxurl, { action: 'simple_page_ordering', id: changes['id'], previd: changes['previd'], nextid: changes['nextid'], start: changes['start'] }, update_simple_ordering_callback );
	} else {
		jQuery.each(changes, function(key,value) {
			if ( jQuery('#inline_'+key).length > 0 ) {
				if ( value['menu_order'] != null ) {
					jQuery('#inline_'+key+' .menu_order').html( value['menu_order'] );
					jQuery('#inline_'+key+' .post_parent').html( value['post_parent'] );
					var post_title = jQuery('#inline_'+key+' .post_title').html();
					var dashes = 0;
	  				while ( dashes < value['depth'] ) { post_title = '&mdash; ' + post_title; dashes++; }
	  				jQuery('tr#post-'+key+' a.row-title').html( post_title );
				} else {
					jQuery('#inline_'+key+' .menu_order').html(value);
				}
			}
		});
