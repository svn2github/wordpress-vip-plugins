 ...
 * and call the customMenu function to mark 'Add New Playbuzz Item' at the admin menu as selected
 */
PbStoryCreatorController.prototype.init = function () {
	...
	_this.view.postPreview.listen(
		function (sender, e) {
			...
			_this.model.previewPost( sender.wp ).then( _this.refreshCreator.bind( _this ), _this.errorHandler.bind( _this ) );

		}
	);

	// listen to click event on publish button
	_this.view.publish.listen(
		function (sender, e) {
	...
	_this.view.savePost.listen(
		function (sender, e) {
			_this.actionRunning( sender.pb );

			_this.model.savePost( sender.wp ).then( _this.refreshCreator.bind( _this ), _this.errorHandler.bind( _this ) );

		}
	);

	//listen to changes on title input.
	_this.view.title.listen(
		function (sender, e) {
...
PbStoryCreatorController.prototype.getAddNewPbButton = function () {

	var $clonedButton = this.$( '.page-title-action' ).clone();
	var href = $clonedButton.attr( 'href' );
	$clonedButton.text( 'Add New Playbuzz Item' )
		.attr( 'href', href + '?pb-story=true' )
		.addClass( 'show pb-page-title-action' );

	return $clonedButton;
};

PbStoryCreatorController.prototype.removeAddNewPbButton = function () {

	this.$( '.pb-page-title-action' ).remove();
...
/**
 * mark 'Add New Playbuzz Item' at the admin menu as selected
...
PbStoryCreatorController.prototype.customTitle = function () {
	...
	//TODO -- translation
	var title = this.editPost ? 'Edit Playbuzz Item' : 'Add New Playbuzz Item';
	var titleInput = 'Enter story title here';
	var button = this.editPost ? this.getAddNewPbButton() : null;
...
PbStoryCreatorController.prototype.refreshCreator = function () {
	this.postSuccessHandler();
	this.view.removePbElements();
	this.$( '#pb-story-creator' ).empty();
	this.removeAddNewPbButton();
	window.pbInitStoryCreator( window, this.$ );
};
