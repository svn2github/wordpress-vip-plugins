function PbStoryCreatorView(jQuery, PbEvent){
	...
	this.elementsToCreate = ['#save-post', '#post-preview', '#publish'];
	var elementsToBind = ['#title'];

	this.$           = jQuery;
	this.newElements = this.generateElements( this.elementsToCreate, true );
...
PbStoryCreatorView.prototype.generateElements = function (elements, createNewElement) {
	...
	for (var i = 0; i < elements.length; i++) {
		...
		var idWithPbPrefix = 'pb-' + element;
		var pb = createNewElement ? wp.clone().prependTo( wp.parent() ).attr( 'id', idWithPbPrefix ) : null;
...
PbStoryCreatorView.prototype.init = function () {
	...
	this.newElements['save-post'].pb.click(
		function (e) {
			e.preventDefault();
	...
	this.newElements['post-preview'].pb.click(
		function (e) {
	...
	this.newElements['publish'].pb.click(
		function (e) {
			e.preventDefault();
...
PbStoryCreatorView.prototype.removePbElements = function removePbElements() {
	this.elementsToCreate.forEach(
		function(elementId){
			var pbButtonsId = '#pb-' + elementId.substring( 1 );
			this.$( pbButtonsId ).remove();
		}
	)
};
