 ...
 * @param action
 * @param wpOriginalElement
 * @returns {Promise}
 */
PbStoryCreatorModel.prototype.performAction = function (action, wpOriginalElement, paramsToAction) {
	...
	action( paramsToAction )
		...
		.then(
			function (res) {
				...
				// keep the item id for future refreshing of the creator
				_this.global.setItemId( res.itemId );
...
PbStoryCreatorModel.prototype.savePost = function (saveButton) {
	...
	return this.performAction( this.pbCreator.publish, saveButton );
...
PbStoryCreatorModel.prototype.previewPost = function (previewButton) {
	...
	return this.performAction( this.pbCreator.publish , previewButton, redirect );
