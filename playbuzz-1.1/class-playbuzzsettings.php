			...
			<?php if ( 'embed' == $playbuzz_active_tab || 'start' == $playbuzz_active_tab ) { ?>
				...
				<form method="post" action="options.php">
					...
					<div class="playbuzz_embed">
						...
						<p class="description description-width"><?php esc_html_e( 'Configuring the account will enable you to: create Playbuzz Item items within WordPress, and search for items you created on Playbuzz.com, even if they were not made public.', 'playbuzz' ); ?></p>
					...
					<div class="playbuzz_embed">
						...
						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[jshead]">
							<input type="checkbox" id="<?php echo esc_attr( $this->get_option_name() ); ?>[info]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[jshead]" value="1" 
																	<?php
																	if ( isset( $options['jshead'] ) && ( '1' == $options['jshead'] ) ) {
...
>
						...
						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[info]">
							<input type="checkbox" id="<?php echo esc_attr( $this->get_option_name() ); ?>[info]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[info]" value="1" 
																	<?php
																	if ( isset( $options['info'] ) && ( '1' == $options['info'] ) ) {
																		echo 'checked="checked"';}
?>
>
						...
						<p class="description indent">
							...
							<?php esc_html_e( 'Note: This preference is only relevant for embedded items. Playbuzz items created within WordPress do not include this information as default.', 'playbuzz' ); ?>
						</p>

						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[shares]">
							<input type="checkbox" id="<?php echo esc_attr( $this->get_option_name() ); ?>[shares]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[shares]" value="1" 
																	<?php
																	if ( isset( $options['shares'] ) && ( '1' == $options['shares'] ) ) {
																		echo 'checked="checked"';}
?>
>
						...
						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[comments]">
							<input type="checkbox" id="<?php echo esc_attr( $this->get_option_name() ); ?>[comments]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[comments]" value="1" 
																	<?php
																	if ( isset( $options['comments'] ) && ( '1' == $options['comments'] ) ) {
																		echo 'checked="checked"';}
?>
>
					...
					<div class="playbuzz_embed">
						...
						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[locale]">
							<?php esc_html_e( 'Create Playbuzz items in', 'playbuzz' ); ?>
							<select id="<?php echo esc_attr( $this->get_option_name() ); ?>[locale]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[locale]">
								<option value="sq-AL" 
								<?php
								if ( isset( $options['locale'] ) && ( 'sq-AL' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Albanian', 'playbuzz' ); ?></option>
								<option value="ar" 
								<?php
								if ( isset( $options['locale'] ) && ( 'ar' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Arabic', 'playbuzz' ); ?></option>
								<option value="zh-CN" 
								<?php
								if ( isset( $options['locale'] ) && ( 'zh-CN' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Chinese (Simplified)', 'playbuzz' ); ?></option>
								<option value="zh-HK" 
								<?php
								if ( isset( $options['locale'] ) && ( 'zh-HK' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Chinese (Traditional)', 'playbuzz' ); ?></option>
								<option value="hr-HR" 
								<?php
								if ( isset( $options['locale'] ) && ( 'hr-HR' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Croatian', 'playbuzz' ); ?></option>
								<option value="cs-CZ" 
								<?php
								if ( isset( $options['locale'] ) && ( 'cs-CZ' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Czech', 'playbuzz' ); ?></option>
								<option value="da-DK" 
								<?php
								if ( isset( $options['locale'] ) && ( 'da-DK' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Danish', 'playbuzz' ); ?></option>
								<option value="nl-NL" 
								<?php
								if ( isset( $options['locale'] ) && ( 'nl-NL' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Dutch', 'playbuzz' ); ?></option>
								<option value="en-US" 
								<?php
								if ( isset( $options['locale'] ) && ( 'en-US' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'English', 'playbuzz' ); ?></option>
								<option value="et-EE" 
								<?php
								if ( isset( $options['locale'] ) && ( 'et-EE' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Estonian', 'playbuzz' ); ?></option>
								<option value="fi-FI" 
								<?php
								if ( isset( $options['locale'] ) && ( 'fi-FI' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Finnish', 'playbuzz' ); ?></option>
								<option value="fr-FR" 
								<?php
								if ( isset( $options['locale'] ) && ( 'fr-FR' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'French', 'playbuzz' ); ?></option>
								<option value="de-DE" 
								<?php
								if ( isset( $options['locale'] ) && ( 'de-DE' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'German', 'playbuzz' ); ?></option>
								<option value="el-GR" 
								<?php
								if ( isset( $options['locale'] ) && ( 'el-GR' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Greek', 'playbuzz' ); ?></option>
								<option value="he-IL" 
								<?php
								if ( isset( $options['locale'] ) && ( 'he-IL' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Hebrew', 'playbuzz' ); ?></option>
								<option value="hu-HU" 
								<?php
								if ( isset( $options['locale'] ) && ( 'hu-HU' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Hungarian', 'playbuzz' ); ?></option>
								<option value="hy-AM" 
								<?php
								if ( isset( $options['locale'] ) && ( 'hy-AM' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Armenian', 'playbuzz' ); ?></option>
								<option value="id-ID" 
								<?php
								if ( isset( $options['locale'] ) && ( 'id-ID' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Indonesian', 'playbuzz' ); ?></option>
								<option value="it-IT" 
								<?php
								if ( isset( $options['locale'] ) && ( 'it-IT' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Italian', 'playbuzz' ); ?></option>
								<option value="ja-JP" 
								<?php
								if ( isset( $options['locale'] ) && ( 'ja-JP' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Japanese', 'playbuzz' ); ?></option>
								<option value="ko-KR" 
								<?php
								if ( isset( $options['locale'] ) && ( 'ko-KR' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Korean', 'playbuzz' ); ?></option>
								<option value="lv-LV" 
								<?php
								if ( isset( $options['locale'] ) && ( 'lv-LV' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Latvian', 'playbuzz' ); ?></option>
								<option value="mn-MN" 
								<?php
								if ( isset( $options['locale'] ) && ( 'mn-MN' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Mongolian (Cyrillic)', 'playbuzz' ); ?></option>
								<option value="nb-NO" 
								<?php
								if ( isset( $options['locale'] ) && ( 'nb-NO' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Norwegian (Bokmål)', 'playbuzz' ); ?></option>
								<option value="pl-PL" 
								<?php
								if ( isset( $options['locale'] ) && ( 'pl-PL' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Polish', 'playbuzz' ); ?></option>
								<option value="pt-BR" 
								<?php
								if ( isset( $options['locale'] ) && ( 'pt-BR' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Portuguese', 'playbuzz' ); ?></option>
								<option value="ro-RO" 
								<?php
								if ( isset( $options['locale'] ) && ( 'ro-RO' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Romanian', 'playbuzz' ); ?></option>
								<option value="ro-RO" 
								<?php
								if ( isset( $options['locale'] ) && ( 'ro-RO' == $options['locale'] ) ) {
									echo 'selected';}
...
><?php esc_html_e( 'Romanian', 'playbuzz' ); ?></option>
								<option value="ru-RU" 
								<?php
								if ( isset( $options['locale'] ) && ( 'ru-RU' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Russian', 'playbuzz' ); ?></option>
								<option value="es-ES" 
								<?php
								if ( isset( $options['locale'] ) && ( 'es-ES' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Spanish', 'playbuzz' ); ?></option>
								<option value="sl-SI" 
								<?php
								if ( isset( $options['locale'] ) && ( 'sl-SI' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Slovenian', 'playbuzz' ); ?></option>
								<option value="sv-SE" 
								<?php
								if ( isset( $options['locale'] ) && ( 'sv-SE' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Swedish', 'playbuzz' ); ?></option>
								<option value="th-TH" 
								<?php
								if ( isset( $options['locale'] ) && ( 'th-TH' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Thai', 'playbuzz' ); ?></option>
								<option value="tr-TR" 
								<?php
								if ( isset( $options['locale'] ) && ( 'tr-TR' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Turkish', 'playbuzz' ); ?></option>
								<option value="en" 
								<?php
								if ( isset( $options['locale'] ) && ( 'en' == $options['locale'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Other', 'playbuzz' ); ?></option>
							</select>
						</label>
					...
					<div class="playbuzz_embed">
						...
						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[embeddedon]">
							...
							<select id="<?php echo esc_attr( $this->get_option_name() ); ?>[embeddedon]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[embeddedon]">
								...
								<?php
								if ( isset( $options['embeddedon'] ) && ( 'content' == $options['embeddedon'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'Posts & Pages Only', 'playbuzz' ); ?></option>
								<option value="all"     
								<?php
								if ( isset( $options['embeddedon'] ) && ( 'all' == $options['embeddedon'] ) ) {
									echo 'selected';}
?>
><?php esc_html_e( 'All pages (singular, archive, ect.)', 'playbuzz' ); ?></option>
							</select>
						</label>
					...
					<div class="playbuzz_embed"  style="display:none;">
						...
						<label for="<?php echo esc_attr( $this->get_option_name() ); ?>[experiment-mode]">
							<input type="checkbox" id="<?php echo esc_attr( $this->get_option_name() ); ?>[experiment-mode]" name="<?php echo esc_attr( $this->get_option_name() ); ?>[experiment-mode]" value="1" 
																	<?php
																	if ( isset( $options['experiment-mode'] ) && ( '1' == $options['experiment-mode'] ) ) {
																		echo 'checked="checked"';}
?>
>
