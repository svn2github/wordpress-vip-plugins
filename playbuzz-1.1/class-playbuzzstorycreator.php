...
class PlaybuzzStoryCreator {
	...
	public function add_story_links_admin_bar( $wp_admin_bar ) {
		...
		$args = array(
			'id' => 'add-new-story',
			'parent' => 'new-content',
			'title' => __( 'Playbuzz Item', 'Playbuzz Item' ),
			'href' => esc_url($this->get_new_post_url()),
			'meta' => array(
	...
	public function add_story_links_admin_menu() {
		...
		add_submenu_page(
			'edit.php',
			__( 'Add New Playbuzz Item', 'Add New Playbuzz Item' ),
			'<span class="pb-icon"></span><span class="pb-menu-name">' . esc_html__( 'New Playbuzz', 'New Playbuzz' ) . '</span>',
			'edit_posts',
			esc_url($this->get_new_post_url())
		);
