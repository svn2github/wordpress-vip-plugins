...
(function($) {
	...
	ooyala.State = media.controller.State.extend({
		...
		insert: function() {
			...
			if (ooyala.settings.account) {
				var api_key = ooyala.settings.alt_accounts[ooyala.settings.account].api_key;
			} else {
				var api_key = ooyala.settings.api_key;
			}
