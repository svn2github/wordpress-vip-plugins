...
jQuery(function($) {
	...
	$ooyala
		...
		// toggle additional playlist
		.on('change', '.ooyala-playlists-toggle', function() {
			$('.ooyala-playlists-options').toggle($(this).is(':checked'));
		})

		// caption checkbox change
		.on('change', 'input[name="ooyala[playlists][caption][]"]', function() {
			if ($('input[name="ooyala[playlists][caption][]"]').filter(':checked').length > 0) {
				$('input[name="ooyala[playlists][captionType]"]').filter('[value=custom]').prop('checked', true);
			} else {
				$('input[name="ooyala[playlists][captionType]"]').filter('[value=none]').prop('checked', true);
			}
		})

		// caption radio change
		.on('change', 'input[name="ooyala[playlists][captionType]"]', function(ev) {
			// uncheck all the custom values
			if (ev.target.value !== 'custom') {
				$('input[name="ooyala[playlists][caption][]"]').prop('checked', false);
			}
		})
	;
